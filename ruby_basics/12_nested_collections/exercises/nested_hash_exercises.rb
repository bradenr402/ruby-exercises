def find_language_information(languages, language_name)
  ...
  # }
  languages.each { |language, info| return info if language == language_name }
end
...
def add_information_about_language(languages, language_name, info_key, info_value)
  ...
  languages.each { |language, info| info[info_key] = info_value if language == language_name }
end
...
def find_beautiful_languages(languages)
  ...
  languages.each do |language, info|
    languages.delete(language) if info[:is_beautiful?] == false
  end
  languages
end
